...
import { Constants, Media, WidgetProps } from "@workspace/common-models";
import {
  Image,
  TextRenderer,
  Link,
...
import { FetchBuilder, getPlanPrice } from "@workspace/utils";
    ...
    <Section theme={overiddenTheme} id={cssId}>
      ...
      >
        {featuredImage && (
          ...
          >
            <Image src={featuredImage.file} alt={featuredImage.caption} />
          </div>
        )}
        <div
        ...
        >
          ...
          >
            {type === Constants.PageType.PRODUCT && !isLeadMagnet && (
              <Preheader theme={overiddenTheme}>
                {getSymbolFromCurrency(state.siteinfo.currencyISOCode)}
                {
                  getPlanPrice(
            ...
            <div className="pb-1 mb-4">
              <Header1 theme={overiddenTheme}>{titleText}</Header1>
            </div>
            {finalDescription && (
              <div
                className={`pb-4 ${
            ...
            {type === Constants.PageType.PRODUCT && isLeadMagnet && (
              <div>
                {((editing && showEditingView === "1") || success) && (
                  <Subheader1 theme={overiddenTheme}>
                    <TextRenderer
                      json={successMessage || defaultSuccessMessage}
                    />
                  </Subheader1>
                )}
                  ...
                  !success && (
                    ...
                    >
                      ...
                      <Button
                        theme={overiddenTheme}
                        type="submit"
                        disabled={state.networkAction || !email}
                      >
                        {buttonCaption || "Get for free"}
                      </Button>
                    </form>
                  )}
              </div>
            ...
            {type === Constants.PageType.PRODUCT && !isLeadMagnet && (
              <Link href={`/checkout?type=course&id=${product.courseId}`}>
            ...
            {type === Constants.PageType.COMMUNITY && (
              <div className="flex flex-col gap-4">
                ...
                >
                  <>
                    <Users className="w-4 h-4" /> {product.membersCount} members
                  </>
                </Text2>
                <Link
