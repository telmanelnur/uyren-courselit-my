...
import { Link, TextRenderer } from "@workspace/components-library";
...
import { Preheader } from "@workspace/page-primitives";
      ...
      <div className={`flex flex-col gap-4`}>
        <div
          className={`flex flex-col ${
            headerAlignment === "center" ? "items-center" : "items-start"
          }`}
        >
          ...
          {pricingSwitcher && (
            <div className="flex items-center gap-2 mb-8 mt-4 text-lg w-full justify-center">
              <div className="flex-1 text-right">
                <Text2 theme={overiddenTheme}>{monthlyPriceCaption}</Text2>
              </div>
              <div className="mt-1">
                <Switch
              ...
              <div className="flex-1">
                <Text2 theme={overiddenTheme}>{yearlyPriceCaption}</Text2>
              </div>
            </div>
          )}
        </div>
        {items && items.length > 0 && (
          ...
          >
            {items.map((item, index) => (
              <PageCard key={index} theme={overiddenTheme} className="">
                <PageCardContent
                  className="h-full flex flex-col"
                  theme={overiddenTheme}
                >
                  <Preheader className="" theme={overiddenTheme}>
                    {item.title}
                  </Preheader>
                  ...
                  <div className="grow flex flex-col gap-4">
                    <Text1 component="span" theme={overiddenTheme}>
                    ...
                    <Link
                      href={
                        pricing === "yearly" && item.action.yearlyHref
