...
import { Course, SiteInfo, WidgetProps } from "@workspace/common-models";
import {
  TextRenderer,
  SkeletonCard,
...
import { Header1, Subheader1, Section } from "@workspace/page-primitives";
    ...
    <Section theme={overiddenTheme} id={cssId}>
      <div className={`flex flex-col w-full gap-4`}>
        <div
          ...
          style={{
            alignItems: headerAlignment === "center" ? "center" : "flex-start",
          }}
        >
          ...
          {productItems.length === 0 && (
            <>
              {Array.from({ length: 3 }).map((_, index) => (
                <SkeletonCard key={index} theme={overiddenTheme} />
              ))}
            </>
          )}
          {productItems.length > 0 && (
              ...
              {productItems.map((course: Course) => (
                <ProductCard
                  ...
                  user={{
                    ...
                    thumbnail: course.user?.avatar?.thumbnail || "",
                  }}
                  image={
                  ...
                  badgeChildren={getBadgeText(course, state.siteinfo)}
                />
              ))}
            </>
          )}
