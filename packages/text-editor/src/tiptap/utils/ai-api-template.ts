...
export async function processTextWithAI(request: AIProcessingRequest): Promise<string> {
  ...
  switch (action) {
    ...
    case "correct":
      return `✅ Grammar & Spelling Corrected: ${text.replace(/\b(\w+)\b/g, (match) => {
...
export const createAIAPIHandler = (customProcessor?: (request: AIProcessingRequest) => Promise<string>) => {
