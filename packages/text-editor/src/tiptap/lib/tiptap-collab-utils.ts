...
export const fetchCollabToken = async () => {
  ...
  // Return null if no collaboration token is provided
  // This allows the editor to work without collaboration features
  if (!TIPTAP_COLLAB_TOKEN) {
    console.log("Collaboration features disabled - no token provided");
    return null;
  }

  console.warn(
    "You are using the example JWT token provided in the Tiptap Cloud dashboard. This is only for local development and should not be used in production.",
  );
...
export const fetchAiToken = async () => {
  ...
  // Return null if no AI token is provided
  // This allows the editor to work without AI features
  if (!TIPTAP_AI_TOKEN) {
    console.log("AI features disabled - no token provided");
    return null;
  }
