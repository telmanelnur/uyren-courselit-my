...
import { File, FileImage, FileText, Music, Search, Upload, Video, X } from "lucide-react";
...
const MediaBrowserContent: React.FC<{
  ...
  const [selectedFileType, setSelectedFileType] = useState<string>(initialFileType ?? "all");
  ...
  const loadMedia = useCallback(async (page: number, reset: boolean = false) => {
    try {
      setLoading(true);
      const query: any = {
    ...
    <div className="space-y-4">
      ...
      <div className="mb-4 space-y-3">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Select value={selectedFileType} onValueChange={handleMimeTypeFilterChange}>
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        xhr.upload.addEventListener('progress', (event) => {
          if (event.lengthComputable) {
            ...
            const totalProgress = Math.round(((i + fileProgress / 100) / selectedFiles.length) * 100);
    ...
    <div className="h-full flex flex-col space-y-6">
      ...
      <div className="text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mb-6">Select images, videos, audio, or PDF files</p>
        </div>
      ...
      {selectedFiles.length > 0 && (
        <div className="space-y-3">
          {selectedFiles.map((file, index) => (
            <div key={`${file.name}-${index}`} className="p-4 bg-blue-50/50 rounded-lg border border-blue-100">
              <h4 className="font-medium mb-2 text-blue-900">Selected File {selectedFiles.length > 1 ? `${index + 1}` : ''}</h4>
              <div className="flex items-center justify-between">
                <span className="truncate flex-1 mr-2 text-sm">{file.name}</span>
...
const MediaBrowserComponent = (props: {
  selectMode?: boolean;
  ...
  const [internalSelectedMedia, setInternalSelectedMedia] = useState<Media | null>(selectedMedia || null);
  const [total, setTotal] = useState(0);
              ...
              {selectMode && (
                <div className="flex items-center justify-between px-4 py-2 border-b">
                  <div className="text-sm text-muted-foreground">
                    {internalSelectedMedia ? `Selected: ${internalSelectedMedia.originalFileName}` : "Select a file from the list"}
                  </div>
                  <Button
                    size="sm"
                    onClick={() => {
