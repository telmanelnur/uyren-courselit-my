...
import { PieChart, Search } from "@workspace/icons";
import {
  DropdownMenu,
  DropdownMenuTrigger,
      ...
      <div className="flex items-center gap-2">
        <DropdownMenu>
          ...
          <SegmentEditor2
            ...
            onDelete={({ selectedSegment, segments: receivedSegments }) => {
              if (receivedSegments) {
                mapSegments(receivedSegments);
              }
              const selectedSeg = segments.find(
                (segment) => segment.segmentId === selectedSegment,
              );
              setInternalFilters([...(selectedSeg?.filter.filters || [])]);
              setInternalAggregator(selectedSeg?.filter.aggregator || "or");
              setActiveSegment(selectedSegment);
              onChange({
                filters: [...(selectedSeg?.filter.filters || [])],
                aggregator: selectedSeg?.filter.aggregator || "or",
        ...
        <div className="flex flex-wrap gap-1 mb-2">
          ...
          {!disabled && (
            <>
              ...
              <Popover open={segmentSaveOpen} onOpenChange={setSegmentSaveOpen}>
                <FilterSave
                  filters={internalFilters}
                  aggregator={internalAggregator}
