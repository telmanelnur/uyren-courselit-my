...
export default function ProductFilterEditor({ onApply }: { onApply: (...args: any[]) => any }) {
    const [condition, setCondition] = useState(USER_FILTER_PRODUCT_HAS);
    const [value, setValue] = useState("");
    const { toast } = useToast();

    // tRPC for products
    const productsQuery = trpc.lmsModule.courseModule.course.list.useQuery({
    ...
    const onSubmit = (e: any) => {
        ...
        if (buttonName === "apply") {
            onApply({
                ...
                value,
                valueLabel:
                    productOptions.find((x) => x.value === value)?.label || "",
            });
        } else {
            onApply();
        }
    ...
    return (
        <Form className="flex flex-col gap-2 p-2" onSubmit={onSubmit}>
            <DropdownMenuLabel>{USER_FILTER_CATEGORY_PRODUCT}</DropdownMenuLabel>
            <Select
                value={condition}
