...
export default function FilterEditor({
  ...
  const categoryKeys = Object.keys(categoriesMap) as UserFilterType[];

  return (
    <DropdownMenuContent className="w-72">
      {!activeCategory && (
        <>
          <DropdownMenuLabel>{USER_FILTER_DROPDOWN_LABEL}</DropdownMenuLabel>
      ...
      {activeCategory && activeCategory === "community" && (
        <CommunitiesFilterEditor onApply={changeFilter} address={address} />
      )}
