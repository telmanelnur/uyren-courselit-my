...
import { USER_FILTER_CHIP_TOOLTIP } from "@/lib/ui/config/strings";
import { useMemo } from "react";
import permissionToCaptionMap from "../permissions-to-caption-map";
import { UserFilter as Filter } from "@workspace/common-models";
  ...
  const displayedValue = useMemo(() => {
    ...
    if (name === "permission") {
      if (!permissionToCaptionMap[value]) {
        throw new Error(
          `Permission ${value} not found in permissionToCaptionMap`,
        );
      }
