...
import { Clipboard } from "@workspace/icons";
...
export default function NewApikey({
  ...
  const createApikey = async (e: FormEvent) => {
    e.preventDefault();
    try {
      const response = await addApikeyMutation.mutateAsync({
        data: {
          name,
        },
      });
      ...
      >
        ...
        {apikey && (
          <div className="flex flex-col gap-2">
            ...
            <p className="text-slate-500">{APIKEY_NEW_GENERATED_KEY_DESC}</p>
            <div className="flex gap-2 mb-4">
              <FormField name="apikey" value={apikey} disabled />
              <IconButton className="px-3" onClick={copyApikey} variant="soft">
