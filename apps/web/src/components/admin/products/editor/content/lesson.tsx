...
import { Media, Profile, Quiz } from "@workspace/common-models";
...
import { Back, Help, Info } from "@workspace/icons";
import {
  Tooltip,
  Link,
  Button,
    ...
    const fetch = new FetchBuilder()
      ...
      .setPayload({
        ...
        variables: {
          ...
          media: media
            ? Object.assign({}, media, {
                file: media.access === "public" ? media.file : null,
              })
            : null,
        },
      })
    ...
    if (lesson?.lessonId) {
      ...
      try {
        ...
        if (response.result) {
          ...
          router.replace(`/dashboard/product/${courseId}?tab=Content`);
        }
      } catch (err: any) {
        toast({
          title: TOAST_TITLE_ERROR,
            ...
            {courseId && (
              <Tooltip title="Go back to content">
                <IconButton variant="soft">
                  <Link href={`/dashboard/product/${courseId}?tab=Content`}>
                    <Back />
                  </Link>
                </IconButton>
              </Tooltip>
        ...
        {![
          ...
          <div>
            <MediaSelector
              ...
              srcTitle={(lesson?.media && lesson?.media.originalFileName) || ""}
              onSelection={(media?: Media) => {
                if (media) {
                  setLesson(
                    Object.assign({}, lesson, {
                      title: lesson?.title || media.originalFileName,
                      media,
                    }),
                  );
