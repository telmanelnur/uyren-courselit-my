...
import { SequenceStatus, SequenceType } from "@workspace/common-models";
...
const isActiveStatus = (status: SequenceStatus): status is "active" =>
  status === "active";
const isDraftOrPausedStatus = (
  status: SequenceStatus,
): status is "draft" | "paused" => status === "draft" || status === "paused";
const hasEmails = (emails: any[]): emails is [any, ...any[]] =>
  emails.length > 0;
...
type SequenceItemType =
  GeneralRouterOutputs["mailModule"]["sequence"]["list"]["items"][number];
                ...
                <TableRow key={broadcast.sequenceId}>
                  ...
                  <TableCell className="text-right">
                    {type === "broadcast" && (
                      <>
                        {isActiveStatus(broadcast.status) &&
                          ...
                          !isDateInFuture(
                            new Date(broadcast.emails[0].delayInMillis),
                          ) && (
                    ...
                    {type === "sequence" && (
                      <>
                        {isDraftOrPausedStatus(broadcast.status) && (
                          <Chip>{capitalize(broadcast.status)}</Chip>
                        )}
                        {isActiveStatus(broadcast.status) && (
                          <Chip className="!bg-black text-white !border-black">
