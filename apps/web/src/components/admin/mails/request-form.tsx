...
const RequestForm = () => {
  ...
  const loadMailRequestStatusQuery =
    trpc.mailModule.mailRequest.getMailRequestStatus.useQuery();
  const updateMailRequestMutation =
    trpc.mailModule.mailRequest.updateMailRequest.useMutation();
  ...
  const loading =
    loadMailRequestStatusQuery.isLoading || updateMailRequestMutation.isPending;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div>
        {status && (
          <p>
            <span className="font-semibold">Status:</span> {capitalize(status)}
          </p>
        )}
        {message && (
          <p>
