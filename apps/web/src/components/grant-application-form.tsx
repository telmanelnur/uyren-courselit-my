...
export function GrantApplicationForm({ onClose }: GrantApplicationFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.intendedTrack) newErrors.intendedTrack = "Intended track is required"
    ...
    if (formData.motivation.length < 100) newErrors.motivation = "Motivation must be at least 100 characters"
    ...
    return (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <Button onClick={onClose} className="w-full bg-brand-primary hover:bg-brand-primary-hover">
              Close
            </Button>
          </CardContent>
        </Card>
      ...
      <Card className="w-full max-w-2xl my-8">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-bold text-black">Grant Application</CardTitle>
              <CardDescription>
                Fill out this form to apply for financial aid. All fields marked with * are required.
              </CardDescription>
            </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Education & Program */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-black">Education & Program</h3>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <SelectTrigger className={errors.educationStatus ? "border-red-500" : ""}>
                  ...
                  {errors.educationStatus && <p className="text-sm text-red-500 mt-1">{errors.educationStatus}</p>}
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFormData((prev) => ({ ...prev, intendedTrack: value }))}
                  >
                    <SelectTrigger className={errors.intendedTrack ? "border-red-500" : ""}>
                  ...
                  {errors.intendedTrack && <p className="text-sm text-red-500 mt-1">{errors.intendedTrack}</p>}
                </div>
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData((prev) => ({ ...prev, aidType: value }))}
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="50-percent">Up to 50% Discount</SelectItem>
                  </SelectContent>
                </Select>
                {errors.aidType && <p className="text-sm text-red-500 mt-1">{errors.aidType}</p>}
              </div>
            </div>

            {/* Motivation & Achievements */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-black">Motivation & Achievements</h3>

              <div>
                <Label htmlFor="motivation">Motivation Statement * (500-1000 characters)</Label>
                <Textarea
                  id="motivation"
                ...
                <div className="flex justify-between items-center mt-1">
                  ...
                  <p className="text-sm text-gray-500">{formData.motivation.length}/1000</p>
                </div>
              </div>

              <div>
                <Label htmlFor="achievements">Achievements & Portfolio (Optional)</Label>
                <Textarea
                  id="achievements"
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  {files.id && <p className="text-sm text-green-600 mt-1">✓ {files.id.name}</p>}
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-brand-primary transition-colors">
                    <Upload className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                    <p className="text-sm text-gray-600">Income statement, benefits letter, etc.</p>
                    <input
                      type="file"
                      accept="image/*,.pdf"
                      onChange={(e) => handleFileUpload("proofOfNeed", e.target.files?.[0] || null)}
                      className="hidden"
                    />
                  </div>
                  {files.proofOfNeed && <p className="text-sm text-green-600 mt-1">✓ {files.proofOfNeed.name}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-2">
              ...
              <Label htmlFor="consent" className="text-sm leading-relaxed">
                I confirm that the information provided is accurate and agree to the privacy policy. I understand that
