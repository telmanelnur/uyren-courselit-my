...
const ComponentScaffold = ({
  ...
  const drawer = (
    <ul className="w-full">
      {items.map((item: ComponentScaffoldMenuItem | Divider, index: number) =>
        item === "divider" ? (
          ...
          <li
            ...
            style={{
              backgroundColor:
                router.asPath === item.href ? "#d6d6d6" : "inherit",
            }}
          ...
          >
            ...
            {item.icon && item.iconPlacementRight && <div>{item.icon}</div>}
          </li>
        ) : (
          <li
            key={index}
      ...
      {!siteinfo.hideCourseLitBranding && showCourseLitBranding && (
        <span className="flex justify-center align-center">
          ...
          >
            Powered by <span className="font-semibold">CourseLit</span>
          </Link>
        </span>
      )}
    </ul>
  ...
  return (
    <div className="flex">
      ...
      <Modal open={open} onOpenChange={(status: boolean) => setOpen(status)}>
        {drawer}
      </Modal>
      {/* MY <Toaster /> ENDMY */}
    </div>
