...
import { CommunityMemberStatus, Constants } from "@workspace/common-models";
import {
  Link,
  PaginatedTable,
  Tooltip,
...
type MemeberType =
  GeneralRouterOutputs["communityModule"]["community"]["getMembers"]["items"][number];
...
export function MembershipList({ id }: { id: string }) {
  ...
  const [selectedRequest, setSelectedMember] = useState<MemeberType | null>(
    null,
  );
  ...
  const updateMemberStatusMutation =
    trpc.communityModule.community.updateMemberStatus.useMutation();
  const updateMemberRoleMutation =
    trpc.communityModule.community.updateMemberRole.useMutation();
  ...
  const updateMemberStatus = async (userId: string) => {
    ...
    try {
      ...
      if (response) {
        ...
        setMembers((members) =>
          members.map((member) =>
            member.user.userId === userId ? response : member,
          ),
        );
      }
    } catch (e: any) {
                  ...
                  <TableRow key={member.user.email}>
                      ...
                      <Link href={`/dashboard/users/${member.user.userId}`}>
                        <div className="flex items-center space-x-2">
                          <Avatar className="h-8 w-8">
                            ...
                            />
                            <AvatarFallback>
                              {(member.user.name || member.user.email).charAt(
                                0,
                              )}
                            </AvatarFallback>
                          </Avatar>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        ...
                        {member.user.userId !== profile.userId && (
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Badge>{capitalize(member.role)}</Badge>
                        {member.user.userId !== profile.userId && (
