...
type Tmp =
  GeneralRouterOutputs["communityModule"]["post"]["list"]["items"][number];
type CommunityPostListItemType = Omit<Tmp, "updatedAt"> & {
  updatedAt: Date;
};
const transformPost = (post: Tmp): CommunityPostListItemType => ({
  ...post,
...
export function CommunityForum({
  ...
  const [posts, setPosts] = useState<CommunityPostListItemType[]>([]);
  ...
  const [postToDelete, setPostToDelete] =
    useState<CommunityPostListItemType | null>(null);
  ...
  const loadPostListQuery = trpc.communityModule.post.list.useQuery(
    {
      filter: {
        communityId: id!,
  ...
  const toggleLikeMutation =
    trpc.communityModule.post.togglePostLike.useMutation({
      onError: (err) => {
        toast({
  ...
  const togglePinMutation = trpc.communityModule.post.togglePinned.useMutation({
    onError: (err) => {
      toast({
        title: TOAST_TITLE_ERROR,
        description: err.message,
  ...
  const handleReportPost = (post: CommunityPostListItemType) => {
    setPostToReport(post);
    setShowReportConfirmation(true);
  };

  const reportPostMutation =
    trpc.communityModule.community.reportCommunityContent.useMutation({
      onSuccess: () => {
        toast({
          title: TOAST_TITLE_SUCCESS,
  ...
  const joinCommunityMutation = trpc.communityModule.community.join.useMutation(
    {
      onSuccess: (data) => {
        setMembership(data);
        setRefreshCommunityStatus((prev) => prev + 1);
        if (data.status === Constants.MembershipStatus.ACTIVE) {
          toast({
            title: TOAST_TITLE_SUCCESS,
          ...
          toast({
            title: TOAST_TITLE_ERROR,
            description:
              data.rejectionReason || "Your request to join is pending.",
            variant: "destructive",
          });
        }
      },
  ...
  if (loaded && !community) {
    return (
      <NotFound resource="Community" backLink="/" backLinkText="Back to Home" />
    );
  }
    ...
    <div className="container mx-auto p-0">
      {!community?.enabled && (
        <div className="bg-red-400 p-2 mb-4 text-sm text-white rounded-md">
          This community is not enabled. It is not visible to your audience
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {formattedLocaleDate(post.updatedAt)} •{" "}
                          ...
                          {membership &&
                            hasCommunityPermission(
                              ...
                              <Button
                                ...
                                onClick={(e) => togglePin(post.postId, e)}
                              >
                    ...
                    >
                      ...
                      <div className="grid gap-4">
                        <div className="flex items-center gap-2 justify-between">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {profile &&
                                profile.userId !== post.user?.userId && (
                                  <DropdownMenuItem
                        ...
                        <div className="space-y-4 max-h-[300px] overflow-y-auto">
                          {post.comments.map((comment) => (
                            <Comment
                              ...
                              comment={comment}
                              onLike={(commentId) =>
                                handleCommentLike(post.id, commentId)
                              }
                              onReply={(commentId, content) =>
                                handleCommentReply(post.id, commentId, content)
                              }
                            />
                          ))}
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            handleNewCommentChange(post.postId, e.target.value)
                          }
                        />
                        <Button onClick={() => handlePostComment(post.postId)}>
                          Post Comment
                        </Button> */}
                        ...
                        <CommentSection
                          ...
                          postId={post.postId}
                          communityId={id!}
                          onPostUpdated={(postId: string, count: number) => {
                            setPosts((prevPosts) =>
                              prevPosts.map((p) =>
                                p.postId === postId
                                  ? {
