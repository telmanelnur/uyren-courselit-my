...
import { Badge, CreditCard, Lock, Shield, ArrowLeft, CheckCircle } from "lucide-react";
...
export default function StripeCheckoutForm() {
    ...
    const { data: course, isLoading: isCourseLoading } = trpc.lmsModule.courseModule.course.publicGetByCourseId.useQuery(
        { courseId: courseId! },
        { enabled: !!courseId }
    );

    const selectedPlan = course?.attachedPaymentPlans?.find(plan => plan.planId === planId);
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.billingAddress.line1) newErrors.billingAddress = { ...newErrors.billingAddress, line1: "Address is required" };
        if (!formData.billingAddress.city) newErrors.billingAddress = { ...newErrors.billingAddress, city: "City is required" };
        if (!formData.billingAddress.state) newErrors.billingAddress = { ...newErrors.billingAddress, state: "State is required" };
        if (!formData.billingAddress.postalCode) newErrors.billingAddress = { ...newErrors.billingAddress, postalCode: "Postal code is required" };
    ...
    const handleAddressChange = (field: string, value: string) => {
        ...
        // Clear error when user starts typing
        if (errors.billingAddress?.[field as keyof typeof formData.billingAddress]) {
            setErrors(prev => ({
                ...prev,
                billingAddress: {
                    ...prev.billingAddress,
    ...
    const getPlanPrice = (plan: any): { amount: number; period: string; isFree: boolean } => {
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <div className="max-w-4xl mx-auto">
                {/* Header */}
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-bold text-[rgb(var(--brand-dark))] mb-2">Complete Your Payment</h1>
                    <p className="text-[rgb(var(--brand-gray))]">Secure payment powered by Stripe</p>
                </div>
                    ...
                    <div className="lg:col-span-1">
                        <Card className="sticky top-4">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <Button
                                    variant="outline"
                                    onClick={() => router.push(`/checkout?id=${courseId}&type=course`)}
                                    className="w-full"
                                >
