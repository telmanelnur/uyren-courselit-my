  ...
  const features = [
    {
      ...
      description:
        "Courses are divided into modules, assignments, and final projects. Clear structure over chaos.",
    },
    {
      icon: "/img/python.svg",
      title: "Built-in practice",
      description:
        "Each topic comes with tasks and case studies. Learning means solving, not just watching.",
    },
    {
    ...
    },
    {
      ...
      title: "Feedback",
      description:
        "Smart chatbot and expert support. Answers come when they're really needed.",
    },
  ];
  ...
  return (
    <div className="min-h-screen font-montserrat">
      ...
      <section className="relative w-full bg-background">
        ...
        <div
          className="md:hidden w-full bg-[url('/img/banner_copy.png')] bg-cover bg-center relative"
          ...
          <div className="max-w-screen-xl mx-auto flex flex-col-reverse md:flex-row items-center gap-[48px] justify-center">
            <div className="max-w-xl z-10 text-center md:text-left space-y-[12px]">
              <h1 className="text-[32px] md:text-[48px] font-[700] text-foreground mb-[12px]">
                <span className="text-brand-primary font-[700]">Create</span> a
                future with real skills
              </h1>
              <p className="text-[16px] md:text-[18px] lg:text-[20px] font-[500] text-muted-foreground mb-[32px]">
                Learn what actually matters: Python, ML, science. No fluff. Just
                challenges, structure, and growth.
              </p>
              ...
              <div className="flex gap-[40px] text-center justify-center md:justify-start">
                {stats.map((stat, index) => (
                  <div key={index} className="w-[96px]">
                    <h2 className="text-[28px] md:text-[36px] font-[700] text-foreground">
                      {stat.number}
                    </h2>
        ...
        <div className="mx-auto max-w-[1280px]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[20px] md:gap-[100px] mb-[10px] md:mb-[60px] items-center">
            <h2 className="text-[42px] font-[700] text-foreground">
              <span className="text-brand-primary">More</span> than just video
              lessons
            </h2>
            <p className="text-[18px] md:text-[20px] text-muted-foreground max-w-[524px] md:pl-[30px] leading-snug">
              Courses built with a system: theory, practice, support. Every
              module is a step toward real growth.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[30px]">
            {features.map((feature, index) => (
              <div
                key={index}
                className="bg-background p-[30px] rounded-[16px] shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:shadow-[0_6px_18px_rgba(0,0,0,0.2)] border border-border transition-all duration-300"
              >
                <div className="flex items-center mb-[20px]">
                  ...
                  <h3 className="text-[22px] font-[600] text-foreground">
                    {feature.title}
                  </h3>
                </div>
                <p className="text-[20px] leading-[1.5] font-[400] text-muted-foreground">
                  {feature.description}
                </p>
              </div>
            ))}
        ...
        <div className="mx-auto max-w-[1280px] rounded-[40px] p-[30px_24px] flex flex-col md:flex-row items-center justify-between gap-[20px]">
          <div className="w-full md:max-w-[560px] space-y-[20px]">
            <h2 className="text-[42px] font-[700] text-foreground break-words">
              <span className="text-brand-primary text-center">Courses</span>{" "}
              that lead to real results
            </h2>
            <Link
              href="/courses"
              className="inline-block w-full sm:w-[80%] bg-background border-[3px] border-brand-primary text-brand-primary text-[24px] font-[700] rounded-[10px] py-[15px] px-[20px] text-center hover:bg-brand-primary hover:text-white transition-all duration-300"
            >
          ...
          <div className="w-full flex flex-wrap gap-[20px] justify-center md:justify-end">
            {courses.map((course) => (
              <Link
                ...
                className="flex-1 max-w-[280px] bg-card rounded-[12px] p-[20px] shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:shadow-[0_6px_18px_rgba(0,0,0,0.2)] border border-border transition-all duration-300"
              >
                <Image
                  src={course.image}
                  alt={course.title}
                  width={280}
                  height={160}
                  className="w-full rounded-[10px] mb-[10px] object-cover"
                />
                <div className="flex items-center gap-[5px] mb-[5px]">
                  <Image
                    ...
                    width={30}
                    height={30}
                    className="w-[30px] p-[2px] border border-muted-foreground rounded-[5px]"
                  />
                  <p className="text-[16px] font-[400] text-foreground">
                    Uyren Academy
                  </p>
                </div>
                <h3 className="text-[20px] font-[700] text-foreground mb-[10px] break-words">
                  {course.title}
                </h3>
                ...
                <div className="bg-card rounded-[16px] border-2 border-transparent group-hover:border-amber-400/60 p-[30px] shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_16px_48px_rgba(0,0,0,0.2)] transition-all duration-500 max-w-[320px]">
                  <div className="flex items-center gap-[8px] mb-[20px]">
                    {[...Array(testimonials[0]?.rating || 5)].map((_, i) => (
                      <Star
                        key={i}
                        className="w-[20px] h-[20px] fill-yellow-400 text-yellow-400"
                      />
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-[16px]">
                    ...
                    <div>
                      <p className="font-[700] text-foreground text-[16px]">
                        {testimonials[0]?.name || ""}
                      </p>
                      <p className="text-[14px] font-[400] text-muted-foreground">
                        {testimonials[0]?.parentType || ""}
                      </p>
                    </div>
                  </div>
                </div>
