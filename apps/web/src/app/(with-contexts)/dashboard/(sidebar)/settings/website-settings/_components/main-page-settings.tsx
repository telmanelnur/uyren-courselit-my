...
const ReviewImportDialog = NiceModal.create<
  ...
  | {
      ...
      data: {
        key: string;
        rating: number;
        author?: { userId: string; name: string; avatar?: any };
      };
    }
>(({}) => {
  const { visible, hide, resolve } = NiceModal.useModal();
...
const mainPageSchema = z.object({
  ...
  featuredCourses: z.array(
    z.object({
      ...
      shortDescription: z.string().max(500, "Description too long"),
      level: z.enum(["beginner", "intermediate", "advanced"]),
      duration: z
        .number()
        .min(0, "Duration must be at least 0")
        .max(1000, "Duration too high"),
      isFeatured: z.boolean(),
      order: z
        .number()
        .min(0, "Order must be at least 0")
        .max(1000, "Order too high"),
    }),
  ),
  featuredReviews: z.array(
    z.object({
      ...
      targetType: z
        ...
        .max(50, "Target type too long"),
      targetId: z.string().max(50, "Target ID too long"),
      order: z
        .number()
        .min(0, "Order must be at least 0")
        .max(1000, "Order too high")
