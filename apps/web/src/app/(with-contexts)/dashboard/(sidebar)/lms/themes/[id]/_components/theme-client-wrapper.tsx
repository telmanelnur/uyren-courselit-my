...
function ThemeContent() {
  ...
  const handleStatusChange = useCallback(
    async (newStatus: BasicPublicationStatus) => {
      if (!theme?._id) return;
            ...
            <div className="flex items-center gap-2">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    {theme?.status === BASIC_PUBLICATION_STATUS_TYPE.DRAFT &&
                      "Draft"}
                    {theme?.status ===
                      BASIC_PUBLICATION_STATUS_TYPE.PUBLISHED && "Published"}
                    {theme?.status === BASIC_PUBLICATION_STATUS_TYPE.ARCHIVED &&
                      "Archived"}
                    {!theme && "Draft"}
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onClick={() =>
                      handleStatusChange(
                        BASIC_PUBLICATION_STATUS_TYPE.DRAFT as BasicPublicationStatus,
                      )
                    }
                    disabled={
                      theme?.status === BASIC_PUBLICATION_STATUS_TYPE.DRAFT
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      handleStatusChange(
                        BASIC_PUBLICATION_STATUS_TYPE.PUBLISHED as BasicPublicationStatus,
                      )
                    }
                    disabled={
                      theme?.status === BASIC_PUBLICATION_STATUS_TYPE.PUBLISHED
                    }
                  >
                    Published
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() =>
                      handleStatusChange(
                        BASIC_PUBLICATION_STATUS_TYPE.ARCHIVED as BasicPublicationStatus,
                      )
                    }
                    disabled={
                      theme?.status === BASIC_PUBLICATION_STATUS_TYPE.ARCHIVED
