...
export default function QuizSubmissions() {
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <div className="font-medium">
                                    {attempt.user?.name || attempt.user?.email || attempt.userId || "Unknown Student"}
                                </div>
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="flex items-center gap-2">
                            ...
                            <Badge variant={attempt.percentageScore && attempt.percentageScore >= 70 ? "default" : "secondary"}>
                                {attempt.percentageScore ? `${attempt.percentageScore}%` : "N/A"}
                            </Badge>
                        </div>
                    );
                },
    ...
    const queryParams = useMemo(() => {
        const parsed: any = {
            ...
            filter: {
                ...
                status: Array.isArray(
                    ...
                    ? (tableState.columnFilters.find((filter) => filter.id === "status")?.value as string[])[0] as "completed" | "graded" | "in_progress" | "abandoned"
                    : undefined,
            }
        };
        ...
        return parsed;
    }, [tableState.sorting, tableState.pagination, tableState.columnFilters, tableState.globalFilter, quiz?._id]);

    const loadSubmissionsQuery = trpc.lmsModule.quizModule.quizAttempt.list.useQuery(queryParams, {
        enabled: !!(quiz?._id),
    });
    ...
    useEffect(() => {
        ...
        setParsedPagination({
            pageCount: Math.ceil((loadSubmissionsQuery.data.total || 0) / (loadSubmissionsQuery.data.meta?.take || 20)),
        });
