...
export default function SubmissionDetailDialog({
    ...
    const { data: questionsData } = trpc.lmsModule.quizModule.quizQuestions.listQuestions.useQuery(
        {
    ...
    return (
        <Dialog open={control.visible} onOpenChange={control.setVisible}>
            <DialogContent className="max-w-6xl h-[90vh] flex flex-col" style={{ maxWidth: '90vw' }}>
                            ...
                            <CardContent>
                                <div className="space-y-6">
                                    {quizData?.questionIds?.map((questionId, index) => {
                                        ...
                                        const answer = submission.answers?.find(a => a.questionId === questionId?.toString())
                                        const isCorrect = answer?.isCorrect
                                        const score = answer?.score || 0
                                        
                                        return (
                                            <div key={questionId?.toString()} className="space-y-4 border rounded-lg p-4">
                                                <div className="flex items-start justify-between">
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-base font-medium mb-3">{question.text}</p>
                                                        ...
                                                        <div className="space-y-3">
                                                            ...
                                                            <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-md border border-green-200 dark:border-green-800">
                                                                <div className="text-sm font-medium text-green-800 dark:text-green-200 mb-2">
                                                                    Correct Answer{question.type === "multiple_choice" && question.options?.filter(opt => opt.isCorrect).length > 1 ? 's' : ''}:
                                                                </div>
                                                                {question.type === "multiple_choice" ? (
                                                                    <div className="space-y-1">
                                                                        {question.options?.filter(opt => opt.isCorrect).map((option, optIndex) => (
                                                                            <div key={optIndex} className="flex items-center gap-2 text-sm text-green-700 dark:text-green-300">
                                                                    ...
                                                                    <div className="space-y-1">
                                                                        {question.correctAnswers?.map((correctAns, ansIndex) => (
                                                                            <div key={ansIndex} className="flex items-center gap-2 text-sm text-green-700 dark:text-green-700">
                                                                                <CheckCircle className="h-4 w-4 text-green-600" />
                                                                                "{correctAns}"
                                                                            </div>
                                                                        ))}
                                                                    ...
                                                                    <div className="space-y-2">
                                                                        {question.type === "multiple_choice" ? (
                                                                            <div className="space-y-1">
                                                                                {Array.isArray(answer.answer) ? (
                                                                                    answer.answer.map((ans: string, i: number) => {
                                                                                        const isCorrectAnswer = question.options?.some(opt => 
                                                                                            opt?.text === ans && opt?.isCorrect
                                                                                        )
                                                                                        return (
                                                                            ...
                                                                            <div className="space-y-1">
                                                                                {Array.isArray(answer.answer) ? (
                                                                                    answer.answer.map((ans: string, i: number) => {
                                                                                        const isCorrectAnswer = question.correctAnswers?.some(correctAns => 
                                                                                            correctAns.toLowerCase().trim() === ans.toLowerCase().trim()
                                                                                        )
                                                                                        return (
                                                                                            <div key={i} className={`flex items-center gap-2 text-sm ${
                                                                                                isCorrectAnswer ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'
                                                                ...
                                                                <div className="flex items-center gap-4">
                                                                    ...
                                                                    {answer?.feedback && (
                                                                        <div className="text-sm">
                                                                            ...
                                                                            <span className="italic">{answer.feedback}</span>
                                                                        </div>
                                                                    )}
                                                                </div>
                                                            </div>
