...
export default async function QuizAttemptPage({ params }: QuizAttemptPageProps) {
  ...
  try {
    // Fetch quiz data
    const quiz = await trpcCaller.lmsModule.quizModule.quiz.publicGetByQuizIdWithQuestions({ quizId: id })
    if (!quiz) {
      notFound()
    }
    ...
    if (serializedAttempt.status === "completed" || serializedAttempt.status === "graded") {
      return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
          <Card className="w-full max-w-2xl mx-auto">
            <CardHeader>
              <CardTitle className="text-center">Quiz Already Completed</CardTitle>
            </CardHeader>
            <CardContent className="text-center">
              <p className="text-gray-600 mb-4">
                This quiz attempt has already been completed.
              </p>
              <Link href={`/quiz/${serializedQuiz._id}/attempts/${serializedAttempt._id}/results`}>
                View Results
              </Link>
            </CardContent>
          </Card>
    ...
    if (serializedAttempt.status === "abandoned") {
      return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
          <Card className="w-full max-w-2xl mx-auto">
            <CardHeader>
              <CardTitle className="text-center text-red-600">Attempt Abandoned</CardTitle>
            </CardHeader>
            <CardContent className="text-center">
              <p className="text-gray-600 mb-4">
