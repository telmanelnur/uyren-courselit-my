...
const MultipleChoiceQuestion = ({ options, currentAnswer, onOptionChange }: {
  options?: Array<{ _id: string; uid: string; text: string; isCorrect?: boolean; order?: number }>
    ...
    {options?.map((option) => (
      <div key={option._id} className="flex items-center space-x-3">
        <Checkbox
          ...
          onCheckedChange={(checked) => onOptionChange(option.uid, checked as boolean)}
        />
        <Label htmlFor={option._id} className="cursor-pointer text-base leading-relaxed">
          {option.text}
        </Label>
      </div>
    ))}
...
export default function QuizInterface({ 
  ...
  const updateCurrentAnswer = useCallback((newAnswer: any) => {
    if (!currentQuestion) {
    ...
    setQuizState(prev => {
      ...
      const hasValidAnswer = newAnswer && (
        Array.isArray(newAnswer) ? newAnswer.length > 0 && newAnswer.some(a => a && a.toString().trim() !== '') : 
        typeof newAnswer === 'string' ? newAnswer.trim() !== '' : 
        true
      )
      if (hasValidAnswer) {
  ...
  const handleMultipleChoiceChange = useCallback((optionUid: string, checked: boolean) => {
  ...
  const navigateToQuestion = useCallback(async (targetIndex: number) => {
    if (targetIndex === currentQuestionIndex || targetIndex < 0 || targetIndex >= initialQuiz.questions.length) {
      return
    }
  ...
  }, [attemptId, currentQuestion, currentAnswer, currentQuestionIndex, toast, router, submitDialog])
  ...
  return (
    <QuizWrapper>
      <Card className="w-full h-full">
              ...
              <div className="border-t bg-gray-50/50 p-6">
                <div className="flex justify-between items-center">
                  ...
                  {currentQuestionIndex === initialQuiz.questions.length - 1 ? (
                    ...
                    <Button 
                      onClick={handleNext}
                      disabled={currentQuestionIndex === initialQuiz.questions.length - 1 || isLoading}
                      size="lg"
                    >
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="grid grid-cols-4 gap-3">
                  {initialQuiz.questions.map((question, index) => {
                    const hasAnswer = quizState.answeredQuestions.has(question._id)
        ...
        <Dialog open={submitDialog.visible} onOpenChange={submitDialog.setVisible}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Confirm Submission</DialogTitle>
              <DialogDescription>
                Are you sure you want to submit this quiz? You cannot go back and edit your answers after submission.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
...
const ShortAnswerQuestion = ({ currentAnswer, onAnswerChange }: {
  currentAnswer: string[]
