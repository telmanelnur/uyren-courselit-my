...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!checkPermission(session.user.permissions, [permissions.manageSite, permissions.manageSettings])) {
        ...
        try {
            if (entityType === "all" || entityType === "courses") {
                if (actions.fixLessonOrder) {
                    results.results.courseLessonOrder = await fixCourseLessonOrder(dryRun);
                }
            ...
            if (entityType === "all" || entityType === "memberships") {
                if (actions.removeInvalidMemberships) {
                    results.results.invalidMemberships = await removeInvalidMemberships(dryRun);
                }
            }
        } catch (error: any) {
            results.errors.push({
...
async function analyzeCourseLessonSync() {
    ...
    for (const course of courses) {
                ...
                if (group.lessonsOrder && group.lessonsOrder.length > 0) {
                    for (const lessonId of group.lessonsOrder) {
                        ...
                        if (!lessonIds.has(lessonId)) {
                            courseIssues.issues.push(`Group "${group.name}": Lesson ${lessonId} not found in Lesson model`);
                            missingLessons++;
                            totalIssues++;
                        }
                        // Check if lesson exists in course.lessons (priority check)
                        if (course.lessons && !course.lessons.includes(lessonId)) {
                            courseIssues.issues.push(`Group "${group.name}": Lesson ${lessonId} not found in course.lessons array`);
                            invalidLessonOrders++;
                            totalIssues++;
                        }
                    }
        ...
        if (course.lessons && course.lessons.length > 0) {
            for (const lessonId of course.lessons) {
                if (!lessonIds.has(lessonId)) {
                    courseIssues.issues.push(`Lesson ${lessonId} in lessons array not found in Lesson model`);
                    missingLessons++;
                    totalIssues++;
                }
            }
    ...
    if (missingLessons > 0) {
        recommendations.push(`Fix ${missingLessons} missing lesson references in courses`);
    }
    if (orphanedLessons > 0) {
        recommendations.push(`Remove ${orphanedLessons} orphaned lessons not in course.lessons`);
    }
    if (invalidLessonOrders > 0) {
        recommendations.push(`Fix ${invalidLessonOrders} group lesson orders not matching course.lessons`);
    }
...
async function analyzeMembershipSync() {
    ...
    for (const membership of memberships) {
        ...
        if (membership.entityType === "course" && !courseIds.has(membership.entityId)) {
    ...
    if (invalidUserReferences > 0) {
        recommendations.push(`Fix ${invalidUserReferences} invalid user references in memberships`);
    }
    if (invalidEntityReferences > 0) {
        recommendations.push(`Fix ${invalidEntityReferences} invalid entity references in memberships`);
    }
...
async function analyzeLessonIntegrity() {
    ...
    for (const lesson of lessons) {
        ...
        if (lesson.groupId) {
            ...
            if (course && course.groups) {
                const groupExists = course.groups.some(g => g.groupId === lesson.groupId);
                if (!groupExists) {
                    lessonIssues.issues.push(`Group ${lesson.groupId} not found in course ${lesson.courseId}`);
    ...
    if (invalidCourseReferences > 0) {
        recommendations.push(`Fix ${invalidCourseReferences} invalid course references in lessons`);
    }
    if (invalidGroupReferences > 0) {
        recommendations.push(`Fix ${invalidGroupReferences} invalid group references in lessons`);
    }
    ...
    for (const course of courses) {
        ...
        for (let i = 0; i < updatedGroups.length; i++) {
            ...
            if (group.lessonsOrder && group.lessonsOrder.length > 0) {
                ...
                const validLessons = group.lessonsOrder.filter(lessonId => validLessonIds.has(lessonId));
                ...
                if (validLessons.length !== group.lessonsOrder.length) {
                    ...
                    results.push({
                        ...
                        removedLessons: group.lessonsOrder.filter(lessonId => !validLessonIds.has(lessonId)),
                        action: dryRun ? "Would remove lessons not in course.lessons" : "Removed lessons not in course.lessons",
                    });
...
async function fixCourseReferences(dryRun: boolean) {
    ...
    for (const course of courses) {
        ...
        const validLessons = course.lessons.filter(lessonId => lessonIds.has(lessonId));
        ...
        if (validLessons.length !== course.lessons.length) {
            ...
            results.push({
                courseId: course.courseId,
                title: course.title,
                removedLessons: course.lessons.filter(lessonId => !lessonIds.has(lessonId)),
                action: dryRun ? "Would remove invalid lessons" : "Removed invalid lessons",
            });
        }
    }
...
async function removeOrphanedLessons(dryRun: boolean) {
    ...
    const orphanedLessons = lessons.filter(lesson => !referencedLessonIds.has(lesson.lessonId));
    ...
    return {
        ...
        action: dryRun ? "Would remove orphaned lessons" : "Removed orphaned lessons",
    };
}
...
async function removeInvalidMemberships(dryRun: boolean) {
    ...
    const invalidMemberships = memberships.filter(membership => {
        ...
        // Check if entity exists for course memberships
        if (membership.entityType === "course" && !courseIds.has(membership.entityId)) return true;
    ...
    return {
        ...
        action: dryRun ? "Would remove invalid memberships" : "Removed invalid memberships",
    };
}
