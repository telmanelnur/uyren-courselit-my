...
export async function POST(req: NextRequest) {
  try {
    ...
    const domain = await getDomain(headers.identifier);
    if (!domain) {
    ...
    Log.error(`Error in payment webhook: ${e.message}`, {
...
async function handleInvoice(
  ...
  if (invoice) {
    ...
    await InvoiceModel.create({
      ...
      paymentProcessorTransactionId: paymentMethod.getPaymentIdentifier(body),
      currencyISOCode,
    });
  }
}
