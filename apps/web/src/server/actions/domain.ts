...
export async function clearDomainManagerCache() {
    try {
        ...
        if(!checkPermission(session.user.permissions, ['setting:manage'])) {
            throw new Error('User not have permission to execute clearDomainManagerCache()');
        }
        ...
        return { success: true, message: 'Domain cache cleared successfully', response };
    } catch (error: any) {
        return { success: false, message: error.message ||  'Failed to clear domain cache', };
    }
}